...
export const Header = () => {
  ...
  const [hidden, setHidden] = useState(true); //Search Button [Open-ON]

  //Dark Mode Settings
  
  //<1> Go to tailwind.config.js add "Dark Mode Class"

  //<2> Create state and localStorage for Dark Mode
  const [darkMode, setDarkMode] = useState( JSON.parse(localStorage.getItem("darkMode")) || false);
  
  //<3> useEffect to load the data from the LocalStorage
  useEffect(() => {
  ...
  //Active and UnActive Class while Cliking [Remember Pseudo Classes]
        ...
        <div className="flex flex-wrap justify-between items-center mx-auto">
         ...
         {/* Logo with Link */}
          <Link to="/" className="flex items-center">
              <img src={Logo} className="mr-2 h-10 sm:h-9" alt="IMdB Logo" />
              <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">IMdB-Movies</span>
          </Link>

          {/* This button toggles Dark Mode SUN and MOON */}
          ...
          {/* Search Button */}
            ...
            <div className="hidden relative md:block">
              ...
              {/* Nav-Bar Open And Close on Mobile-Mode */}
            </div>
